...
-- Custom types
create type public.app_permission as enum ('workspaces.delete', 'campaigns.delete');
...
comment on table public.workspaces is 'A workspace that runs advertisements.';
...
comment on table public.user_workspaces is 'A user-workspace mapping table.';
...
create or replace function is_user_authorized_for_workspace(user_id uuid, workspace_id uuid)
returns boolean as
$$
  begin
    return exists (
      select 1 from user_workspaces
      where user_workspaces.user_id = is_user_authorized_for_workspace.user_id
      and user_workspaces.workspace_id = is_user_authorized_for_workspace.workspace_id
    );
  end;
$$ language plpgsql security definer;
...
CREATE POLICY "Allow users to view only their workspaces"
...
create policy "User can access only their workspaces" on public.workspaces
  for select using (is_user_authorized_for_workspace((select auth.uid()), id));

-- Allow only authenticated users to insert new workspaces
...
-- Allow users to update only workspaces they are mapped to
CREATE POLICY "Allow users to update their workspaces"
ON public.workspaces
FOR UPDATE
USING (is_user_authorized_for_workspace((select auth.uid()), id));

CREATE POLICY "Allow users to delete their workspaces"
ON public.workspaces
FOR DELETE
USING (is_user_authorized_for_workspace((select auth.uid()), id));

CREATE OR REPLACE FUNCTION add_user_workspace_mapping()
RETURNS TRIGGER AS
$$
BEGIN
...
CREATE TRIGGER trigger_add_user_workspace_mapping
AFTER INSERT ON workspaces
FOR EACH ROW
EXECUTE FUNCTION add_user_workspace_mapping();
